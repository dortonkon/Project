...
public class PrimarySchool extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void Play() {
        ...
        if (SP.getString(key1, "").equals("")) {
            SharedPreferences.Editor editor = SP.edit();
            editor.putString(key1, "0");
            editor.commit();
        }
        if (Integer.parseInt(SP.getString(key1, "")) < Scores) {
            SP = getSharedPreferences("save", MODE_PRIVATE);
            SharedPreferences.Editor editor = SP.edit();
            editor.putString(key1, String.valueOf(Scores));
            editor.commit();
        }
        if (SP.getString(key1, "").equals("0")) {
            record.setText("Рекорд: 0");
        } else record.setText("Рекорд: " + SP.getString(key1, ""));
        ...
        switch ((int) (Math.random() * 2)) {
            case 0:
                ...
                secondNum = (int) (Math.random() * 21);
                rez = firstNum + secondNum;
                example.setText(firstNum + " + " + secondNum + " = ");
                break;
            case 1:
                secondNum = (int) (Math.random() * 21);
                firstNum = (int) (Math.random() * (21 - secondNum) + secondNum);
                rez = firstNum - secondNum;
                example.setText(firstNum + " - " + secondNum + " = ");
                break;
        }
    ...
    protected void scoreCount() {
        Scores += 10 + (int) (Math.random() * 11) + (int) (s / 2);
