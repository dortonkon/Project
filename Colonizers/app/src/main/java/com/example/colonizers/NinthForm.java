...
public class NinthForm extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ninth_form);
    ...
    protected void Play() {
        ...
        if (SP.getString(key3, "").equals("")) {
            SharedPreferences.Editor editor = SP.edit();
            editor.putString(key3, "0");
            editor.commit();
        }
        if (Integer.parseInt(SP.getString(key3, "")) < Scores) {
            SP = getSharedPreferences("save", MODE_PRIVATE);
            SharedPreferences.Editor editor = SP.edit();
            editor.putString(key3, String.valueOf(Scores));
            editor.commit();
        }
        if (SP.getString(key3, "").equals("0")) {
            record.setText("Рекорд: 0");
        } else record.setText("Рекорд: " + SP.getString(key3, ""));
        ...
        switch ((int) (Math.random() * 6)) {
            case 0:
                firstNum = (int) (Math.random() * 81);
                secondNum = (int) (Math.random() * 81);
                rez = firstNum + secondNum;
                example.setText(firstNum + " + " + secondNum + " = ");
                break;
            case 1:
                secondNum = (int) (Math.random() * 81);
                firstNum = (int) (Math.random() * (81 - secondNum) + secondNum);
                rez = firstNum - secondNum;
                example.setText(firstNum + " - " + secondNum + " = ");
                break;
            case 2:
                secondNum = (int) (Math.random() * 16);
                firstNum = (int) (Math.random() * 16);
                rez = firstNum * secondNum;
                example.setText(firstNum + " • " + secondNum + " = ");
                break;
            case 3:
                firstNum = (int) (Math.random() * 16);
                secondNum = ((int) (Math.random() * 16) + 1);
                rez = firstNum;
                example.setText(firstNum * secondNum + " : " + secondNum + " = ");
                break;
            case 4:
                firstNum = (int) (Math.random() * 21);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.pause:
                ...
                i.putExtra("key", String.valueOf(3));
                startActivity(i);
                break;
